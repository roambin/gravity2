package servlet;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.json.JSONObject;

import entity.EntityAttributeTable;

/**
 * Servlet implementation class ServletChatFriendName
 */
@WebServlet("/ServletChatFriendName")
public class ServletChatFriendName extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ServletChatFriendName() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("servlet>"+this.getClass().getName());
		JSONObject jsonObject=new JSONObject();
		String hyid=null,nname=null;
		hyid=request.getParameter("hyid");
		EntityAttributeTable table=new EntityAttributeTable("nname","user");
		table.setSqlCondition("userid", hyid);
		table.sqlSelect();
		if(table.setObject()) {
			nname=table.getValue("nname");
		}
		jsonObject.put("nname", nname);
		response.getWriter().print(jsonObject);
	}
}
